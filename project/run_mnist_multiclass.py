...
class Network(minitorch.Module):
    ...
    def forward(self, x):
        ...
        X3 = minitorch.avgpool2d(self.out, ((4, 4)))
        # Flatten channels, height, width
        X3 = X3.view(BATCH, 392)
        X4 = self.Linear1.forward(X3).relu()
        if self.training:
            X5 = minitorch.dropout(X4, 0.25)
        X5 = self.Linear2.forward(X5)
        X7 = minitorch.logsoftmax(X5, 1)
        return X7
