...
class CNNSentimentKim(minitorch.Module):
    ...
    def forward(self, embeddings):
        ...
        X1 = self.conv1.forward(emb).relu()
        X2 = self.conv2.forward(emb).relu()
        X3 = self.conv3.forward(emb).relu()
        # Apply max over time??
        X4 = minitorch.nn.max(X1, 2) + minitorch.nn.max(X2, 2) + minitorch.nn.max(X3, 2)
        X5 = self.Linear.forward(X4)
        if self.training:
            X5 = minitorch.nn.dropout(X5, self.Dropout)
        X7 = X5.sigmoid()
        return X7.view(X7.shape[0])
