...
def tensor_conv1d(
    ...
    if reverse is False:
        ...
        for b in prange(batch_):
            for k in prange(out_channels):
                for i in prange(out_width):
                    Res = 0.0
                    # Loop through the weights
                    for l in range(in_channels_):
                        for m in range(kw):
                            Position = i + m
                            if Position < out_width:
                                Weight_Pos = k * s1[0] + l * s1[1] + m * s1[2]
                                Input_Pos = b * s2[0] + l * s2[1] + Position * s2[2]
                                Res += weight[Weight_Pos] * input[Input_Pos]
                    out_pos = (
                        b * out_strides[0] + k * out_strides[1] + i * out_strides[2]
                    )
                    out[out_pos] = Res
    else:
        for b in range(batch_):
            for k in range(out_channels):
                # Each slide will correspond to a value of i
